...
class Encoder(nn.Module):
    def __init__(self, target_len):
        ...
        self.encoder = nn.GRU(input_size=64, hidden_size=512, num_layers=2, bias=True, batch_first=False, dropout=0,bidirectional=True)

        self.transformer_encoded = nn.TransformerEncoder(nn.TransformerEncoderLayer(1024, 8, 256, dropout=0), 2)
        self.transformer_att_dense = nn.Linear(64, 1024)
        self.transformer_att = nn.TransformerEncoder(nn.TransformerEncoderLayer(1024, 8, 256, dropout=0), 4)
...
class Decoder(nn.Module):
    def __init__(self, features_out, target_len):
        ...
        self.decoder_cell=nn.GRU(input_size=1024, hidden_size=512, num_layers=2, bias=True, batch_first=False, dropout=0, bidirectional=False)

        self.dense=DenseLayers(features_in=512, features_out=features_out)
    ...
    def forward(self, encoded_seq, attention_seq, context):
        ...
        #exit(0)
        pred=torch.empty(self.target_len, batch_size, 512)
